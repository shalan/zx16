...
class ZX16:
    ...
    def step(self):
        # take a pending hardware interrupt at the instruction boundary
        if self.ie and self.irq_pending:
            self.epc = self.pc
            self.ie = False
        ...
        if op == 0:  # R-type
            ...
            elif f3 == 0x2: nextpc = self.epc; self.ie = True                     # RETI
            ...
            else: raise Exception(f"bad SYS f3 {f3} @ {self.pc:04x}")
